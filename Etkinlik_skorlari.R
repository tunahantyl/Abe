...
library(openxlsx)
library(xlsx)
directory.name="D:/OneDrive - BAKIRÇAY ÜNİVERSİTESİ/Research/SFA/python/" # Change this directory
...
for (dist.name in dist.names[1]){ # simdilik hnormal dagilim kullanalim.
  ...
  for (year.name in year.names){
    data0 <- read_excel(paste0(directory.name,"Hamveri 26102024.xlsx"),sheet=paste0(year.name)) # Not: İlgili yılın verisi yıl.xlsx formatında kaydedilmemli
    ...
    for (depvar in output.names){
      ...
      err0=try({
        sink(file=paste0(directory.name,'SFA Regression Tables with 13 vars/SFA_',year.name,'_',depvar,'_',dist.name,'.txt'))
        print(summary(res))
        sink()
      },silent=T)
